...
B<child-monitor> does not write to F<pidfile>.

If I<pidfile> cannot be written to, B<child-monitor> will exit.
...
B<child-monitor> exits the next time that the child exits.
...
B<child-monitor> when the child exits.  If we would have restarted it before
...
SIGUSR2 tells B<child-monitor> to begin monitoring the child process.  If the
child is still running, B<child-monitor> does nothing.  If it is not running,
it is restarted.
